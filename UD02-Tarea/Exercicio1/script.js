...
function avaliarNota(nota) {
    ...
    //Mediante el bucle else if vamos definiendo todos los casos posibles.

    if (notaParseada>=0 && notaParseada<5) {
        resultado = (`Insuficiente :( / ${notaParseada}` );       
    }else if (notaParseada>=5 && notaParseada<=5.99){
        resultado = (`Suficiente ¬¬ / ${notaParseada}`);
    }else if (notaParseada>=6 && notaParseada<=6.99){
        resultado = (`Ben/ ${notaParseada}`);
    }else if (notaParseada>=7 && notaParseada<=8.99){
        resultado = (`¡Notable!/ ${notaParseada}`);
    }else if (notaParseada>=9 && notaParseada<=10) {
        resultado =(`¡¡Sobresaliente!!/ ${notaParseada}`);
    }else if (notaParseada<0 || notaParseada>10){
